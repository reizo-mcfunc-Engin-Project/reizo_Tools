...
# 何にも持っていないならガラスに。
    ...
    data modify storage reizo_tools:_ DisplayTool.Block set value "glass"
...
#> デバッグ用
    # ストレージに見たいストレージを入れる。
    data modify storage reizo_mcfunc_engin:test OPS.NBT set from storage reizo_tools:_ DisplayTool.Block
    # 召喚したブロックの名前をアウトプット
    execute \
