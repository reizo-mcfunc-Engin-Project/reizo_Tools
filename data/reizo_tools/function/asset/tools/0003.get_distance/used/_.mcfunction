#> reizo_tools:asset/tools/0003.get_distance/used/_
#
# 
#
# @within function reizo_tools:asset/tools/0003.get_distance/tick/_

# 音を出す
playsound item.spyglass.use player @s ~ ~ ~ 1 1 1

# シフトを押していないなら、ループ開始
execute \
unless score @s reizo_mcfunc_Engin.Sneaking matches 1.. \
anchored eyes \
positioned ^ ^ ^ run \
function reizo_tools:asset/tools/0003.get_distance/used/loop/_

# シフトを押しているなら
execute \
if score @s reizo_mcfunc_Engin.Sneaking matches 1.. run \
function reizo_tools:asset/tools/0003.get_distance/used/get_n/_

# 距離測定失敗したときの処理、get_nは含まれず
execute \
unless score @s reizo_mcfunc_Engin.Sneaking matches 1.. \
unless entity @s[tag=reizo_mcfunc_Engin.Tools.GetDis.GetDisSuccess] run \
function reizo_tools:asset/tools/0003.get_distance/used/fail

# 剥奪
tag @s remove reizo_mcfunc_Engin.Tools.GetDis.GetDisSuccess