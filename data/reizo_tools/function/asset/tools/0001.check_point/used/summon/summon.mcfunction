#> reizo_tools:asset/tools/0001.check_point/used/summon/summon
#
...
function reizo_mcfunc_engin:api/object/summon.m {ID:"tools.check_point_marker",namespace:"reizo_tools"}
...
# 言葉でお知らせ！
    ...
    function reizo_tools:asset/tools/0001.check_point/used/summon/get_marker_pos
    # 取得出来てないならエラー出す。
    execute \
    unless data storage reizo_tools:_ MarkerPos run \
    function reizo_mcfunc_engin:error/fail_data_get {Type:"マーカー",Data:"Pos"}
    # 取得出来たらマクロに代入して、tellraw。
    execute \
    if data storage reizo_tools:_ MarkerPos run \
    function reizo_tools:asset/tools/0001.check_point/used/summon/tell.m with storage reizo_tools:_ MarkerPos
